...
rm -Rf $outputdir/GramNegative_Output/Secreted; mkdir -p $outputdir/GramPositive_Output/Secreted/Final_IDs
...
$signalp -t gram+ -f short $lineonefasta > $outputdir/GramPositive_Output/signalp_pos.short_out #keep
...
sort Secreted/secreted_IDs.output | uniq > Secreted/uniquesecreted_IDs.output
grep -A1 -f Secreted/uniquesecreted_IDs.output $lineonefasta > Secreted/grampositive_secreted.fasta #grep for all the secreted IDs to find their corresponding fasta sequence and save to file with all gram positive and secreted sequences.
...
awk -F " " '{if ($7==0)print $2}' Secreted/Final_IDs/temp.output > Secreted/Final_IDs/grampositive_secreted_TMHMM_IDS.output #keep (Ids of secreted gram positive IDs with NO TM domains)
...
echo  "Now Processing SignalP (Gram -)...\n"
$signalp -t gram- -f short $lineonefasta > $outputdir/GramNegative_Output/signalp_neg.short_out #keep
...
grep -A1 -f Secreted/secreted_IDs.output $lineonefasta > Secreted/gramnegative_secreted.fasta #grep for all the secreted IDs to find their corresponding fasta sequence and save to file with all gram negative and secreted sequences.
...
awk -F " " '{if ($7==0)print $2}' Secreted/Final_IDs/temp.output > Secreted/Final_IDs/gramnegative_secreted_TMHMM_IDS.output #keep (Ids of secreted gram positive IDs with NO TM domains)
rm Secreted/Final_IDs/temp.output #remove
grep -A1 -f Secreted/Final_IDs/gramnegative_secreted_TMHMM_IDS.output $lineonefasta > Secreted/Final_IDs/gramnegative_secreted_TMHMM_IDS.fasta #This gets the corresponding sequences to all ids with no TM domains that are secreted and gram positive
